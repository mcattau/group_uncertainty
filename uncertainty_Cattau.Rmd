...
# now we have the tall trees for each plot. We need to calculate the canopy diameter for each plot as our measure of canopy closure
tall_trees<-site_insitu[site_insitu$stemheight]
insitu_canopy_closure<-site_insitu %>% 
  group_by(plotid) %>% 
  summarise(mean(stemheight))
# take the df and group by plot id, and for each group, give max value
names(insitu_mean_stem_height)<-c("plotid", "insitu_meanHt") # rename columns
