...
# plot of difference between LiDAR and ground ~ canopy closure
# take the df and group by plot id, and for each group, give max value

```

merged.max.height$diff_max<-merged.max.height$SJER_lidarCHM-merged.max.height$insitu_maxHt

merged.mean.height$diff_mean<-merged.mean.height$SJER_lidarCHM-merged.mean.height$insitu_meanHt

par(mfrow=c(2,1))
plot(merged.max.height$diff_max~canopy_closure$canopy_area)
abline(lm(merged.max.height$diff_max~canopy_closure$canopy_area))
plot(merged.mean.height$diff_mean~canopy_closure$canopy_area)
abline(lm(merged.mean.height$diff_mean~canopy_closure$canopy_area))
